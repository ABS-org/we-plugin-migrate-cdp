 ...
 * 2.: run in terminal:
 *    node node_modules/we-plugin-migrate-cdp/bin/9-relato-sem-autor-sql.js
 */
  ...
  return loadSails(function afterLoadSails(err, sails) {
    ...
    Term.find().exec(function(err, terms) {
      ...
      terms.forEach(function(t){
    	...
    	var sql = "SELECT n.`nid`, n.`type`, n.`uid`, n.`status`,  n.`created`,  n.`changed`, n.`tnid`, title_field.`title_field_value` AS title, field_descricao.`field_descricao_value` AS field_descricao,field_imagem_de_destaque.`field_imagem_de_destaque_fid` AS field_imagem_de_destaque,field_experiencia_desenvolvida.`field_experiencia_desenvolvida_value` AS field_experiencia_desenvolvida,field_experiencia_funcionamento.`field_experiencia_funcionamento_value` AS field_experiencia_funcionamento, field_experiencia_catespecificas.`field_experiencia_catespecificas_tid` AS  field_experiencia_catespecificas, field_experiencia_desafios.`field_experiencia_desafios_value` AS field_experiencia_desafios,field_experiencia_novidades.`field_experiencia_novidades_value` AS field_experiencia_novidades, field_experiencia_observacoes.`field_experiencia_observacoes_value` AS field_experiencia_observacoes, field_premiado_na_v_mostra.`field_premiado_na_v_mostra_value` AS field_premiado_na_v_mostra FROM `node` AS n LEFT JOIN `field_data_title_field` AS title_field ON title_field.entity_id=n.nid LEFT JOIN `field_data_field_descricao` AS field_descricao ON field_descricao.entity_id=n.nid LEFT JOIN `field_data_field_imagem_de_destaque` AS field_imagem_de_destaque ON field_imagem_de_destaque.entity_id=n.nid LEFT JOIN `field_data_field_experiencia_desenvolvida` AS field_experiencia_desenvolvida ON field_experiencia_desenvolvida.entity_id=n.nid LEFT JOIN `field_data_field_experiencia_funcionamento` AS field_experiencia_funcionamento ON field_experiencia_funcionamento.entity_id=n.nid LEFT JOIN `field_data_field_experiencia_catespecificas` AS field_experiencia_catespecificas ON field_experiencia_catespecificas.entity_id=n.nid LEFT JOIN `field_data_field_experiencia_desafios` AS field_experiencia_desafios ON field_experiencia_desafios.entity_id=n.nid LEFT JOIN `field_data_field_experiencia_novidades` AS field_experiencia_novidades ON field_experiencia_novidades.entity_id=n.nid LEFT JOIN `field_data_field_experiencia_observacoes` AS field_experiencia_observacoes ON field_experiencia_observacoes.entity_id=n.nid LEFT JOIN `field_data_field_premiado_na_v_mostra` AS field_premiado_na_v_mostra ON field_premiado_na_v_mostra.entity_id=n.nid WHERE `status`=1 AND `type`='relato_de_experi_ncia_dab_curado'";
